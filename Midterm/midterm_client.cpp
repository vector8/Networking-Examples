// Chris Carmichael and Zac Hills
...
float blueTransX = 100.f, blueTransY = 0.f;
...
void receive()
{
	...
	while (running)
	{
		...
		else if (error == 0)
		{
			...
			blueTransX = otherTrans[0];
			blueTransY = otherTrans[1];

			std::cout << "RECV: " << blueTransX << "," << blueTransY << std::endl;
		}
	}
}
...
void DisplayCallbackFunction(void)
{
	...
	// blue box
	glTranslatef(blueTransX, blueTransY, -200.f);
...
void TimerCallbackFunction(int value)
{
	...
	// update our delay clock
	clock_t currTime = clock();

	double duration = (currTime - startTime) / (double)CLOCKS_PER_SEC;	// in SECONDS
	duration *= MILLIS_PER_SECOND;	// convert to millis

	if (sendPacket && duration > packetDelay)
	{
		//////// SOCKETS ////////////////////
...
void init()
{
	...
	sendto(s, connectMsg.c_str(), connectMsg.length(), 0, (struct sockaddr *) &sockAddr, slen);
	int error = WSAGetLastError();
	if (error != WSAEWOULDBLOCK && error != 0)
	{
		std::cout << "Error code : " << error << "\n";
